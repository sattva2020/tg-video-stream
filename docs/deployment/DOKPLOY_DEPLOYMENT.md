...
> Важно: для `frontend` используем домены Dokploy (авто‑лейблы). Не дублируем кастомные Traefik‑лейблы в compose, иначе Traefik не сможет выбрать сервис и отдаст 404.
