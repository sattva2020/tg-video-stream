...
## Recent Changes
- 008-auth-page-design: Added TypeScript 5.4 + React 18 + Vite 5 (frontend), CSS custom properties для темизации. + TailwindCSS, React Three Fiber (3D фон), Aceternity UI, Magic UI, Hero UI, Zustand (state), i18next для текста.
- 008-auth-page-design: Added TypeScript 5.x, React 18 + TailwindCSS, Framer Motion, React Three Fiber, i18next, React Hook Form, Zod
- 007-user-approval: Added Python 3.11 (backend) / Node 20+ + React 18 + TypeScript (frontend) + FastAPI, SQLAlchemy, Alembic, PostgreSQL (backend); Vite, React, TypeScript, Playwright (frontend)
