#!/usr/bin/env python3
"""
Vimeo Import Verification Script

This script helps verify the database state after a Vimeo album import.
It checks import jobs, playlists, playlist items, and activity logs.

Usage:
    python scripts/verify_vimeo_import.py [--job-id JOB_ID] [--user-id USER_ID]
...
Examples:
    ...
    python scripts/verify_vimeo_import.py

    # Verify specific import job
    python scripts/verify_vimeo_import.py --job-id 123e4567-e89b-12d3-a456-426614174000

    # Verify latest import for specific user
    python scripts/verify_vimeo_import.py --user-id 123e4567-e89b-12d3-a456-426614174000
"""
...
class ImportVerifier:
    """Verifies Vimeo album import database state"""
    ...
    def verify_import_job(self, job_id: Optional[str] = None, user_id: Optional[str] = None) -> dict:
        ...
        query = "SELECT * FROM import_jobs WHERE platform = 'vimeo'"
        ...
        if job['platform'] == 'vimeo':
            print("  ✓ Platform is 'vimeo'")
            checks_passed += 1
        else:
            print(f"  ✗ Platform is '{job['platform']}', expected 'vimeo'")
    ...
    def verify_playlist(self, import_job_id: str) -> dict:
        ...
        # Check 2: Content type is vimeo
        total_checks += 1
        if playlist['content_type'] == 'vimeo':
            print("  ✓ Content type is 'vimeo'")
            checks_passed += 1
        else:
            print(f"  ✗ Content type is '{playlist['content_type']}', expected 'vimeo'")
    ...
    def run_full_verification(self, job_id: Optional[str] = None, user_id: Optional[str] = None):
        ...
        print("\n" + "="*80)
        print("VIMEO ALBUM IMPORT VERIFICATION")
        print("="*80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Verify Vimeo album import database state",
