# Implementation Plan - Feature 010: Telegram Auth & Multichannel
...
- Integration tests for `TelegramAuthService` (mocking Pyrogram).
- Unit tests for `SystemdService` (checking file generation).

### Manual Verification
- Verify `systemctl --user status` shows correct units.
- Verify encrypted data in DB cannot be read without key.
- Verify local file playback works.
