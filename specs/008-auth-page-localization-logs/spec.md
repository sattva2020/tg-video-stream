...
Обязательные поля: `code`, `hint`.

Контрактное уточнение (важно): API исторически принимал либо `message`, либо `message_key` — любой из них может присутствовать или оба могут отсутствовать (в этом случае UI показывает `auth.server_error`).

Планируемая эволюция контракта: чтобы сделать обработку клиентом детерминированной и упростить локализацию, бэкенд ДОЛЖЕН заполнять `message_key`, где это возможно, в дополнение к или вместо серверного `message`. Это ДОЛЖЕН (не ДОЛЖЕН) для обеспечения обратной совместимости со старыми клиентами: серверы ДОЛЖНЫ продолжать принимать случаи, когда `message_key` отсутствует, и клиенты ДОЛЖНЫ следовать цепочке резервирования: 1) отображать `message`, если предоставлено (серверная локализованная строка), 2) иначе разрешить `message_key` через i18n, 3) иначе показать `auth.server_error`.

Если команда решит перейти на более строгий контракт (сервер ДОЛЖЕН предоставлять `message_key`), это будет разрывом контракта и ДОЛЖЕН быть обработан явно как версия API с заметками о миграции и планом поэтапного внедрения (тесты + развертывание). См. "Заметки о миграции контракта" ниже.
...
### Заметки о миграции контракта

- Обратная совместимость: не изменяйте ответы сервера с сегодняшнего дня без обновления спецификации и добавления контрактных тестов. Любое изменение, которое сделает `message_key` обязательным, должно сопровождаться явным планом миграции с развертыванием, проверками совместимости клиента и версионированием API.
- Рекомендуемый стартовый путь миграции: серверы ДОЛЖНЫ как можно скорее выдавать `message_key` (неразрушающее). Добавьте контрактные тесты, которые принимают любой формат, и добавьте небольшой журнал аудита для ответов, не содержащих `message_key`, чтобы команда могла найти отсутствующие случаи перед переходом на MUST.
 - Обязательные артефакты для разрыва: обновленная спецификация + CHANGELOG, pytest контрактное изменение, которое не проходит, когда `message_key` отсутствует (только после согласованного релиза), обновление клиентской библиотеки с четким поведением обработки ошибок и флагом opt-in в CI.
