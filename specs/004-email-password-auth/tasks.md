...
## Фаза 3 — Тесты (Unit & Integration)
 ...
 - [X] Написать unit-тесты для `auth_service` (pytest)
 - [X] Написать API-интеграционные тесты для эндпоинтов auth (FastAPI TestClient)
 - [X] Добавить тест для edge-cases: слабые пароли, повторная регистрация, неверный токен сброса
